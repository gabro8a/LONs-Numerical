...
from pyDOE import *
import problems as BF
import perturbation as PR


# Compute the average escapes given a step size
def AverageEscapes( Func, D, domain, step, digits, tinit ) :
    RUN = 100
    SUBS = 30
    my_method = 'L-BFGS-B'
    my_digits = 4
...
def main():
    ...
    func_name = [ 'Schwefel2_26']
    #func_name = [ 'Ackley4']
    #func_name = ['Griewank']
    #func_name = ['SpreadSpectrumRadarPollyPhase']
    D = [ 3, 5, 8 ]

    #func_name = ['SSCRuspini']
    #D = [ 6, 8, 10 ]
    ...
    for i in range( len( func_name ) ):
        ...
        for j in range( len( D ) ) :
            ...
            while dig <= digits and best_found == False :
                inc = inc / 10.0
                while True :
                    avg = AverageEscapes( Func, D[ j ], domain, test_step, digits, tinit )
