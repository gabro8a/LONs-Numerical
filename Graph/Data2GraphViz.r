...
plotNet3D <-function(N, z, ewidth, vsize, asize, mylay) {
  ...
  rglplot(N, layout = mylayout3D, edge.width = ewidth, edge.arrow.size = asize, vertex.label = NA, vertex.size=vsize)
}
...
trace <- read.table(file, sep= "", header=TRUE, colClasses=c("integer", "integer", "character", "integer","character"))
...
MLON <- LON
V(MLON)$Count<-1
gnn <- subgraph.edges(LON, which(f2==f1), delete.vertices=FALSE)     #TRUE Network with the same fitness value
...
ls_ncol <- "royalblue3"              # non-global sink (bottom of funnel)
myalpha = 255
palt <- c("pink","lightskyblue")
palt = apply(col2rgb(palt), 2, function(x){rgb(red=x[1], green=x[2],blue=x[3],alpha=myalpha, maxColorValue=255)})
...
sing <- sum(strength(graph=CMLON, vids=igs, mode="in", loops=FALSE), na.rm=TRUE)
sinl <- sum(strength(graph=CMLON, vids=ils, mode="in", loops=FALSE), na.rm=TRUE)
gstrength <-  sing/(sing+sinl)
gbasins<-foreach(sink=igs, .packages=c("igraph","foreach"), .inorder=TRUE) %do% {
  subcomponent(graph=CMLON,v=sink,mode="in")
}
v <- unique(array(as.numeric(unlist(gbasins))))
gfunnel <- length(v)/vcount(CMLON)  # proportion of nodes that reach a global optimum
...
# 2D Visualization
png(file = paste0(output_file,'_lon.png'))
plot(LON, vertex.label=NA, vertex.size=vs, edge.width=ew, edge.arrow.size=0.2, layout=lonlay)
png(file = paste0(output_file,'_cmlon.png'))
plot(CMLON, vertex.label=NA, vertex.size=vs2, edge.width=ew2, edge.arrow.size=0.2, layout=lonlay2)
...
######### LON #########
plotNet3D(N = LON, z = zcoord, ewidth = ew, vsize = vs, asize = 1, mylay = lonlay)
# rotating to nice positioning
rgl.viewpoint(180, 90)
# size of image
par3d(windowRect = c(20, 30, 800, 800))
# saving
rgl.snapshot(paste0(output_file,'_3D_lon.png'), fmt = 'png')

# # saving gif of rotation
# M <- plotNet3D(N = LON, z = zcoord, ewidth = ew, vsize = vs, asize = 1, mylay = lonlay)
# # rotating to nice positioning
# rgl.viewpoint(180, 90)
# # size of image
# par3d(windowRect = c(20, 30, 800, 800))
# 
# # GIF
# movie3d( spin3d(rpm=3), duration=10,dir="./", clean=TRUE )
# # movie3d( spin3d(rpm=3),movie=paste0(output_file,'_lon'), duration=10,dir="./", clean=TRUE )
# play3d( spin3d(rpm=3), duration=10)

######### CMLON #########
plotNet3D(N = CMLON, z = zcoord2, ewidth = ew2, vsize = vs2, asize = 1, mylay = lonlay2)
# rotating to nice positioning
rgl.viewpoint(180, 90)
# size of image
par3d(windowRect = c(20, 30, 800, 800))
# saving
rgl.snapshot(paste0(output_file,'_3D_cmlon.png'), fmt = 'png')
# rotating to nice positioning
rgl.viewpoint(180, 90)


# # saving gif of rotation
# M <- plotNet3D(N = CMLON, z = zcoord, ewidth = ew, vsize = vs, asize = 1, mylay = lonlay)
# # rotating to nice positioning
# rgl.viewpoint(180, 90)
# # size of image
# par3d(windowRect = c(20, 30, 800, 800))
...
# # GIF
# movie3d( spin3d(rpm=3), duration=10,dir="./", clean=TRUE )
# # movie3d( spin3d(rpm=3),movie=paste0(output_file,'_cmlon'), duration=10,dir="./", clean=TRUE )
# play3d( spin3d(rpm=3), duration=10)